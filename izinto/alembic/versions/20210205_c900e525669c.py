...
def upgrade():
    ...
    conn.execute("INSERT INTO container_base (id, title, description, image, polymorphic_type)"
                 "(SELECT id, title, description, image, 'collection' FROM collection)")
    ...
    dashboard_id = conn.execute(
        "select * from dashboard order by id DESC").first().id
    collection_id = conn.execute(
        "select * from collection order by id DESC").first().id
    container_id = max(collection_id, dashboard_id)

    op.drop_column('collection', 'image')
    op.drop_column('collection', 'title')
    op.drop_column('collection', 'description')
    op.create_index(op.f('ix_container_base_id'),
                    'container_base', ['id'], unique=False)
    ...
    op.drop_constraint('variable_name_dashboard_id_key',
                       'variable', type_='unique')
    op.drop_constraint('user_dashboard_pkey',
                       'user_dashboard', type_='primary')
    ...
    for dashboard in conn.execute("select * from dashboard order by id"):
        old_id = dashboard.id
        conn.execute("INSERT INTO container_base (id, title, description, image, polymorphic_type)"
                     "SELECT %s, title, description, image, 'dashboard' FROM dashboard where id = %s" % (container_id, old_id))
        # update id and foreign keys
        conn.execute("update dashboard set id = '%s' where id = %s" %
                     (container_id, old_id))
        conn.execute("update chart set dashboard_id = '%s' where dashboard_id = %s" % (
            container_id, old_id))
        conn.execute("update query set dashboard_id = '%s' where dashboard_id = %s" % (
            container_id, old_id))
        conn.execute("update single_stat set dashboard_id = '%s' where dashboard_id = %s" % (
            container_id, old_id))
        conn.execute("update user_dashboard set dashboard_id = '%s' where dashboard_id = %s" % (
            container_id, old_id))
        conn.execute("update variable set dashboard_id = '%s' where dashboard_id = %s" % (
            container_id, old_id))
        container_id += 1
    ...
    op.create_primary_key('user_dashboard_pkey', 'user_dashboard', [
    ...
    op.alter_column('variable', 'dashboard_id',
                    existing_type=sa.INTEGER(), new_column_name='container_id')
    op.create_unique_constraint('variable_name_container_id_key', 'variable', [
                                'name', 'container_id'])
...
def downgrade():
    ...
    op.drop_constraint('fk_variable_container_id',
                       'variable', type_='foreignkey')
    op.drop_constraint('variable_name_container_id_key',
                       'variable', type_='unique')
    op.alter_column('variable', 'container_id',
                    existing_type=sa.INTEGER(), new_column_name='dashboard_id')
    ...
    op.drop_constraint('fk_dashboard_container_id',
                       'dashboard', type_='foreignkey')
    ...
    conn.execute("""
        ...
        title=container_base.title, description=container_base.description, image=container_base.image
        FROM container_base WHERE dashboard.id = container_base.id;
    """)
    ...
    conn.execute("""
        ...
        title=container_base.title, description=container_base.description, image=container_base.image
        FROM container_base WHERE collection.id = container_base.id;
    """)

    op.drop_constraint('fk_collection_container_id',
                       'collection', type_='foreignkey')
